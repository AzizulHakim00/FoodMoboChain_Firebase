...
public final class FirebaseService {
    ...
    @Nullable public FirebaseUser firebaseUser() { return auth.getCurrentUser(); }
    @Nullable public String uid() { FirebaseUser user = firebaseUser(); return user == null ? null : user.getUid(); }
    ...
    public boolean isAdminEmail(@Nullable String email) { return email != null && ADMIN_EMAIL.equalsIgnoreCase(email.trim()); }
    ...
    public void loadCurrentUser(UserCallback callback) {
        ...
        users().child(uid).get().addOnCompleteListener(task -> {
            AppUser user = task.isSuccessful() && task.getResult().exists() ? task.getResult().getValue(AppUser.class) : null;
